  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center bg-white overflow-hidden p-4 sm:p-6">
      ...
      <div className="w-full max-w-3xl mx-auto relative z-10">
        <h1 className="text-3xl sm:text-4xl font-satoshi text-[#F24C00] mb-8 text-center mt-8">Leaderboard</h1>
        {loading ? (
          <div className="text-center text-gray-500 font-satoshi text-lg my-12">Loading leaderboard...</div>
        ) : error ? (
          <div className="text-center text-red-500 font-satoshi text-lg my-12">{error}</div>
        ) : (
        <>
        <div className="flex flex-col sm:flex-row gap-6 justify-center mb-12">
          {topThree.map((entry, idx) => (
            ...
            >
              <div className={`text-3xl mb-2 font-satoshi ${idx === 0 ? 'text-yellow-500' : idx === 1 ? 'text-gray-500' : 'text-orange-500'}`}>
                {idx === 0 ? '🥇' : idx === 1 ? '🥈' : '🥉'}
              </div>
              <div className="font-satoshi text-xl text-[#F24C00] mb-1 truncate w-full" title={entry.name}>{entry.name}</div>
              <div className="font-satoshi text-lg text-black">{entry.totalVotes} votes</div>
            </div>
          ))}
        </div>
        <h2 className="text-2xl font-satoshi text-[#F24C00] mb-6 text-center">All Entries</h2>
          ...
          {otherEntries.map((entry, idx) => (
            <div key={entry.id} className="bg-white/90 rounded-2xl shadow-md border border-[#F24C00]/20 p-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <span className="font-satoshi text-lg text-gray-500 w-6 text-center">{idx+1}</span>
                <div className="font-satoshi text-lg text-[#F24C00]">{entry.name}</div>
              </div>
              <div className="text-right">
                <div className="font-satoshi text-base text-black mb-2">{entry.totalVotes} votes</div>
                <button
                  onClick={() => handleVote(entry.id)}
                  disabled={voteLoading === entry.id}
                  className="relative z-10 px-6 py-2 rounded-full bg-gradient-to-b from-[#fba41b] to-[#fff3e0] shadow text-black font-satoshi text-sm flex items-center gap-2 border-2 border-[#fba41b]/60 hover:scale-105 transition-transform duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
                >
